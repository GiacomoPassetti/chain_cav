...
Nmax, L, g, U, Omega, t, Nfer, dt, d =10, 20, 1, 0.5, 1, 1, 10, 0.05, 2
sites = sites(L,Nmax)
ps= product_state(L)
psi=psi(sites,ps)
...
def Bos_ferm_bond(sites, Nmax, L, g, U, Omega, t, Nfer, dt, d, options, trunc_param):
    ...
    V1=np.reshape(V, [s.shape[0]*4, 4])
    W1=np.reshape(W1, [1,Nmax+1,Nmax+1,s.shape[0]])
    ...
    W2=np.reshape(W2, [W1.shape[1], 2, 2, s.shape[0]])
    W2=np.transpose(W2, [0,3,1,2])
    W3=np.reshape(W3,[s.shape[0], 2, 2, 1] )
...
M=Bos_ferm_bond(sites, Nmax, L, g, U, Omega, t, Nfer, dt, d, options, trunc_param)
for i in range(5):
